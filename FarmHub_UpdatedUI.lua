...
-- KEYBIND TOGGLE
-- ==========================================
-- Change this to whatever key you'd like to use to show/hide the hub.
-- Full list of options: https://create.roblox.com/docs/reference/engine/enums/KeyCode
local ToggleKey = Enum.KeyCode.F2

UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    if gameProcessedEvent then return end -- ignore keypresses while typing in a textbox, etc.
...
-- ==========================================
-- DRAG FUNCTIONALITY (now also drags the shadow)
-- ==========================================
...
-- Modern button with rounded corners, subtle gradient, and hover tween
local function CreateButton(parent, name, callback)
    ...
    AddStroke(btn, Theme.Accent, 1, 0.7)
...
CreateButton(ScrollFrame, "Teleport + Farm (5k Dummy)", function()
    task.spawn(function()
        ...
        local dummiesFolder = map:FindFirstChild("5k_dummies")
        if not dummiesFolder then return end
        ...
        for i = 1, 50 do
            local children = dummiesFolder:GetChildren()
            if #children >= dummyIndex then
                targetDummy = children[dummyIndex]
                if targetDummy:FindFirstChild("Humanoid") and targetDummy:FindFirstChild("HumanoidRootPart") then
                    break
                end
            end
            task.wait(0.1)
        ...
        if not targetDummy then
            warn("ERROR: Dummy index " .. dummyIndex .. " not found! Try changing the index number.")
            return
        end

        local targetHumanoid = targetDummy.Humanoid
        local targetRoot = targetDummy.HumanoidRootPart

        root.CFrame = targetRoot.CFrame * CFrame.new(0, 5, 0)
        task.wait(0.2)

        while not stopAll do
            if not targetDummy.Parent or targetHumanoid.Health <= 0 then
                warn("Target died! Stopping.")
                break
            end
...
CreateButton(ScrollFrame, "Fireball Farm", function()
    task.spawn(function()
        ...
        while not stopAll do
            local dummies = GetCurrentDummyFolder()
            if not dummies then task.wait(0.5) continue end
            ...
            if nearest and nearest:FindFirstChild("HumanoidRootPart") then
                pcall(function()
                    remote:FireServer(nearest.HumanoidRootPart.Position, "NewFireball")
                end)
            end
...
CreateButton(ScrollFrame, "Lightning Ball Farm", function()
    task.spawn(function()
        ...
        while not stopAll do
            ...
            if nearest and nearest:FindFirstChild("HumanoidRootPart") and nearest.Humanoid.Health > 0 then
                pcall(function()
                    remote:FireServer(nearest.HumanoidRootPart.Position, "NewLightningball")
                end)
            end
...
AddCorner(stopBtn, Theme.CornerSmall)
stopBtn.MouseButton1Click:Connect(StopAllScripts)

print("Farm Hub GUI Loaded (Modern UI). Check your screen.")
